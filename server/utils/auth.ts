...
export function verifyToken(token: string): AuthUser | null {
  try {
    ...
    const decoded = jwt.verify(token, runtimeConfig.jwtSecret || 'your-secret-key') as any
...
export async function requireCompanyAccess(event: H3Event, companyId: string, requiredRole?: string): Promise<AuthUser> {
