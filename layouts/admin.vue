        ...
        <div class="border-b border-white/10 p-6">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
              <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-lg font-semibold uppercase">
                {{ companyInitials }}
              </div>
              <div class="space-y-1">
        ...
        <header class="sticky top-0 z-10 border-b border-slate-200 bg-white/80 backdrop-blur">
          <div class="flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h1 class="text-lg font-semibold text-slate-900 sm:text-xl">{{ activeNavLabel }}</h1>
                <p class="text-xs text-slate-500 sm:text-sm">{{ companyName }}</p>
              </div>
            </div>
            <div class="flex items-center gap-3">
              ...
              <div class="flex h-10 w-10 items-center justify-center rounded-full bg-slate-900/90 text-xs font-semibold uppercase text-white sm:h-11 sm:w-11">
                {{ userInitials }}
              </div>
            </div>
          </div>
...
const navItems = [
  ...
  { label: 'Acompanhamentos', to: '/admin/sides', icon: 'i-heroicons-queue-list' },
]
...
const companyName = computed(() => authStore.user?.company?.name ?? 'Restaurante')
...
const userInitials = computed(() => getInitials(authStore.user?.name ?? userName.value))
