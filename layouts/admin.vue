...
const navItems = computed(() => {
  ...
  // Fallback para ler do localStorage direto (solicitado)
  // if (!role && import.meta.client) {
  //   try {
  ...
  // Se não houver role (ex: durante o SSR), retorna vazio para evitar "flash" de menus errados.

  const companyType = authStore.user?.company?.type;
  if (companyType === "delivery" && items.length) {
