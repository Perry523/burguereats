<template>
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-1">
          <div class="flex items-center">
            <nuxt-img src="/logo-restaurante.jpg" alt="Logo" width="70" height="70"/>
          </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="lg:grid lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start lg:gap-10">
        <div>
          <slot />
        </div>
          ...
          <div class="sticky top-24 space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
                ...
                >
                  <div class="flex items-start gap-4">
                    ...
                    <div class="flex flex-1 flex-col gap-1">
                      ...
                      <p v-if="selectionSummary(item)" class="text-xs text-slate-500">
            ...
            <div class="space-y-4 border-t border-slate-200 pt-4">
              <div class="flex items-center justify-between">
                ...
                <span class="text-xl font-bold text-slate-900">{{ currencyFormatter.format(cartTotal) }}</span>
              </div>
              <UButton
                size="lg"
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="isCartPanelOpen" class="fixed inset-0 z-50 lg:hidden">
          <div class="absolute inset-0 bg-slate-950/60" @click="closeCartPanel"></div>
          <div class="absolute inset-y-0 right-0 flex h-full w-full max-w-xs flex-col rounded-l-3xl bg-white shadow-2xl">
            ...
            <div class="border-t border-slate-100 px-6 py-5 space-y-3">
              <div class="flex items-center justify-between">
                <span class="text-sm font-semibold text-slate-600">Total</span>
                <span class="text-lg font-bold text-slate-900">{{ currencyFormatter.format(cartTotal) }}</span>
              </div>
              <UButton
                size="lg"
                class="w-full justify-center"
...
const categories = useState<Array<{ id: string; name: string }>>('menuCategories', () => [
  { id: 'todos', name: 'Todos' },
])
...
const cartPlaceholderImage = 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=400&q=80'
