    ...
    filtered = filtered.filter((d) => {
      ...
      if (d.categories && d.categories.length > 0) {
        return d.categories.some(
          (c) =>
            c.id === categoryFilter.value || c.slug === categoryFilter.value,
        );
      }
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredDishes.value.length / itemsPerPage.value)),
);
...
const columns = [
  { key: "dish", label: "Prato", sm: true },
  { key: "categories", label: "Categorias" },
  { key: "price", label: "Preço", type: "currency", sm: true },
];
...
const syncCategoryOptions = (records: DishRecord[]) => {
  ...
  for (const record of records) {
    if (Array.isArray(record.categories) && record.categories.length) {
      ...
      const existing = defaultCategoryOptions.find(
  ...
  categoryOptions.value = Array.from(map.values()).sort((a, b) =>
  ...
  const categories = Array.isArray(dish.categories)
    ? [...dish.categories]
        .map((category) => ({
          id: category.id,
          name:
            category.name ?? formatCategoryLabel(category.slug ?? category.id),
...
const fetchResources = async (id: string | undefined) => {
  ...
  try {
    const [dishResponse, sideResponse] = await Promise.all([
      $fetch<{ success: boolean; data?: DishRecord[] }>(
        `/api/dishes?companyId=${id}`,
      ),
      $fetch<{ success: boolean; data?: SideCategoryRecord[] }>(
        `/api/side-categories?companyId=${id}`,
      ),
    ]);
    const fetchedDishes = Array.isArray(dishResponse?.data)
      ? dishResponse.data.map(normalizeDish)
      : [];
    dishes.value = fetchedDishes;
    syncCategoryOptions(fetchedDishes);
    const fetchedSides = Array.isArray(sideResponse?.data)
      ? sideResponse.data
      : [];
