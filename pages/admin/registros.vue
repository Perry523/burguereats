<template>
  <div class="h-full flex flex-col pt-0 md:py-4 max-w-7xl mx-auto w-full px-2 sm:px-6 lg:px-8 gap-4 overflow-auto">

    <!-- Filters card -->
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden shrink-0">
      <!-- Row 1: company + biker + type + paid filter -->
      <div class="flex flex-wrap items-stretch divide-x divide-gray-100 border-b border-gray-100">
        <!-- Company -->
        <label class="flex items-center gap-2 px-3 py-2.5 flex-1 min-w-[160px] hover:bg-gray-50/70 transition-colors cursor-pointer group">
          <span class="shrink-0 w-7 h-7 rounded-lg bg-emerald-50 flex items-center justify-center">
            <UIcon name="i-ph-storefront-duotone" class="w-4 h-4 text-emerald-500" />
          </span>
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 leading-none mb-0.5">Empresa</p>
            <select
        ...
        <!-- Biker -->
        <label class="flex items-center gap-2 px-3 py-2.5 flex-1 min-w-[160px] hover:bg-gray-50/70 transition-colors cursor-pointer group">
          <span class="shrink-0 w-7 h-7 rounded-lg bg-violet-50 flex items-center justify-center">
            <UIcon name="i-ph-motorcycle" class="w-4 h-4 text-violet-500" />
          </span>
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 leading-none mb-0.5">Entregador</p>
            <select
              v-model="filterBiker"
              class="w-full text-sm font-semibold text-gray-800 bg-transparent border-none p-0 focus:ring-0 cursor-pointer truncate appearance-none"
              @change="onFilterChange"
            >
              ...
              <option v-for="b in bikers" :key="b.id" :value="b.id">{{ b.name }}</option>
            </select>
          </div>
        </label>

        <!-- Type -->
        <label class="flex items-center gap-2 px-3 py-2.5 flex-1 min-w-[140px] hover:bg-gray-50/70 transition-colors cursor-pointer group">
          <span class="shrink-0 w-7 h-7 rounded-lg bg-amber-50 flex items-center justify-center">
            <UIcon name="i-ph-funnel-duotone" class="w-4 h-4 text-amber-500" />
          </span>
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 leading-none mb-0.5">Tipo</p>
            <select
              v-model="filterType"
        ...
        <!-- Paid status -->
        <label class="flex items-center gap-2 px-3 py-2.5 flex-1 min-w-[140px] hover:bg-gray-50/70 transition-colors cursor-pointer group">
          <span class="shrink-0 w-7 h-7 rounded-lg bg-sky-50 flex items-center justify-center">
            <UIcon name="i-ph-check-circle-duotone" class="w-4 h-4 text-sky-500" />
          </span>
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400 leading-none mb-0.5">Status</p>
            <select
              v-model="filterPaid"
              class="w-full text-sm font-semibold text-gray-800 bg-transparent border-none p-0 focus:ring-0 cursor-pointer appearance-none"
              @change="onFilterChange"
      ...
      <div class="flex items-center gap-2 px-3 py-2">
        ...
        <div class="flex-1 flex items-center justify-center gap-2">
          <span class="w-5 h-5 rounded-md bg-sky-50 flex items-center justify-center shrink-0">
          ...
          >
            <option v-for="w in weeksInMonth" :key="w.label" :value="w.label">{{ w.label }}</option>
          </select>
        </div>
    ...
    <div class="flex flex-wrap gap-3 shrink-0">
      ...
      <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2 shadow-sm">
        ...
        <span class="text-sm font-bold text-gray-800">{{ formatCurrency(totalGross) }}</span>
      </div>
      <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2 shadow-sm">
        <span class="w-2 h-2 rounded-full bg-amber-400"></span>
        <span class="text-xs text-gray-500">Adiantamentos:</span>
        <span class="text-sm font-bold text-amber-700">{{ formatCurrency(totalAdvances) }}</span>
      </div>
      <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2 shadow-sm">
        <span class="w-2 h-2 rounded-full bg-red-400"></span>
        <span class="text-xs text-gray-500">Pendentes:</span>
        <span class="text-sm font-bold text-red-700">{{ formatCurrency(totalPending) }}</span>
      </div>
    </div>

    <!-- Table -->
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex-1 min-h-0 flex flex-col">
      <!-- Loading -->
      <div v-if="isLoading && records.length === 0" class="flex-1 flex items-center justify-center py-20 text-gray-400 gap-3">
        <UIcon name="i-heroicons-arrow-path" class="w-6 h-6 animate-spin" />
        <span class="text-sm">Carregando registros...</span>
      </div>

      <!-- Empty -->
      <div v-else-if="!isLoading && records.length === 0" class="flex-1 flex flex-col items-center justify-center py-20 gap-3 text-gray-400">
      ...
      <div v-else class="overflow-auto flex-1">
        <table class="w-full text-sm text-left">
          <thead class="text-xs text-gray-600 uppercase bg-gray-50 border-b border-gray-200 sticky top-0">
            <tr>
              <th class="px-4 py-3 whitespace-nowrap">Data</th>
              <th class="px-4 py-3 whitespace-nowrap">Entregador</th>
              <th class="px-4 py-3 whitespace-nowrap hidden md:table-cell">Empresa / Tipo</th>
              <th class="px-4 py-3 text-center whitespace-nowrap hidden sm:table-cell">Entregas</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="px-4 py-3 whitespace-nowrap">
                <span class="text-gray-700 font-medium">{{ formatDateBR(r.date) }}</span>
              </td>

              <!-- Biker -->
              <td class="px-4 py-3">
                <p class="font-semibold text-gray-900 text-sm">{{ r.biker_name }}</p>
              </td>
              ...
              <td class="px-4 py-3 hidden md:table-cell">
                ...
                <span v-else class="text-gray-600 text-xs">{{ r.company_name }}</span>
              </td>

              <!-- Deliveries -->
              <td class="px-4 py-3 text-center hidden sm:table-cell">
                <span v-if="!r.is_advance" class="text-gray-600 text-sm">{{ r.total_deliveries || 0 }}</span>
              ...
              <td class="px-4 py-3 text-center">
                <span
                  ...
                  :class="r.is_paid ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'"
                >
                  <UIcon :name="r.is_paid ? 'i-heroicons-check-circle' : 'i-heroicons-clock'" class="w-3 h-3" />
      ...
      <!-- Pagination -->
      <div v-if="totalCount > 0" class="flex items-center justify-between px-4 py-3 border-t border-gray-100 shrink-0 bg-white">
        <span class="text-xs text-gray-500">
          {{ (currentPage - 1) * perPage + 1 }}–{{ Math.min(currentPage * perPage, totalCount) }} de {{ totalCount }} registros
        </span>
        <div class="flex items-center gap-1">
          ...
          <span class="text-xs font-semibold text-gray-700 px-2">Pág. {{ currentPage }}</span>
          <button
            @click="goToPage(currentPage + 1)"
...
const weekRange = ref({ from: toISODate(initMonday), to: toISODate(initSunday) });
...
const weeksInMonth = computed(() => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (sun.getMonth() < pickerMonth.value) { mon.setDate(mon.getDate() + 7); continue; }
...
function shiftWeek(dir: 1 | -1) {
  ...
  const match = weeksInMonth.value.find((w) => toISODate(w.from) === weekRange.value.from);
...
// Init week label
const initLabel = weeksInMonth.value.find((w) => toISODate(w.from) === weekRange.value.from)?.label ?? weeksInMonth.value[0]?.label ?? "";
...
const totalGross = computed(() =>
  records.value.filter((r) => !r.is_advance).reduce((acc, r) => acc + (Number(r.amount) || 0), 0)
);
const totalAdvances = computed(() =>
  records.value.filter((r) => r.is_advance).reduce((acc, r) => acc + (Number(r.amount) || 0), 0)
);
const totalPending = computed(() =>
  records.value.filter((r) => !r.is_paid && !r.is_advance).reduce((acc, r) => acc + (Number(r.amount) || 0), 0)
);
