<template>
  <div class="h-[calc(100vh-140px)] flex flex-col gap-4 pt-6">
    ...
    >
      <template #filter>
        <div class="w-full px-5 pb-4 flex items-center justify-between gap-2">
          ...
          <h1 class="text-2xl font-bold text-gray-800 hidden lg:block mr-2">Minhas Entregas</h1>
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <button 
              ...
              class="flex items-center justify-center p-2.5 text-gray-500 hover:text-primary transition-colors bg-white rounded-lg border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary"
              title="Atualizar"
            >
              <UIcon name="i-heroicons-arrow-path" :class="['h-5 w-5', isLoading ? 'animate-spin' : '']" />
            </button>
      ...
      <template #customer="{ row }">
        <div class="max-w-xs">
          <p class="text-sm font-semibold text-gray-900">{{ row.customer_name }}</p>
          <div class="flex items-center gap-2 mt-1 sm:hidden">
             <!-- On mobile, show an inline status pill -->
             <span
               :class="[
    ...
    <!-- Order Details Modal -->
    <BaseDialog v-model="showOrderModal" title="Detalhes do Pedido" :large="true">
      ...
      <div v-else-if="selectedOrder" class="p-4 sm:p-6 space-y-6">
        ...
        <div class="flex items-center justify-between bg-gray-50 p-4 rounded-lg">
           <div>
             ...
             <span class="font-mono text-gray-900 font-medium">#{{ selectedOrder.id.substring(0, 8) }}</span>
           </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div class="border border-gray-200 rounded-lg p-4">
             ...
             <p class="text-sm text-gray-500">{{ selectedOrder.customer_phone || 'Sem telefone' }}</p>
          </div>
          <div class="border border-gray-200 rounded-lg p-4">
             <h3 class="text-sm font-semibold text-gray-800 mb-2">Endereço de Entrega</h3>
             <p class="text-sm text-gray-700 whitespace-pre-line">{{ selectedOrder.customer_address || 'Não informado' }}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="border border-gray-200 rounded-lg overflow-hidden">
            <div class="divide-y divide-gray-200">
              <div v-for="item in selectedOrder.items" :key="item.id" class="p-4 flex justify-between items-center bg-white">
                <div>
            ...
            <div class="bg-gray-50 p-4 border-t border-gray-200 flex justify-between items-center">
               ...
               <span class="text-xl font-bold text-primary">{{ currencyFormatter.format(selectedOrder.total) }}</span>
            </div>
          </div>
        </div>

        <!-- Notes -->
        <div v-if="selectedOrder.notes" class="border border-amber-200 bg-amber-50 rounded-lg p-4">
           <h3 class="text-sm font-semibold text-amber-800 mb-1">Observações</h3>
           <p class="text-sm text-amber-900 whitespace-pre-line">{{ selectedOrder.notes }}</p>
        </div>
      </div>
    </BaseDialog>
...
const filteredDeliveries = computed(() => {
  ...
  if (search.value) {
    ...
    filtered = filtered.filter(d => 
      ...
      (d.customer_address && d.customer_address.toLowerCase().includes(term)) ||
...
const fetchDeliveries = async () => {
    ...
    try {
        const response = await $fetch<{ success: boolean; data: any[] }>(`/api/bikers/deliveries?bikerId=${bikerProfile.value.id}`);
        if (response.success) {
...
const openOrderModal = async (orderId: string) => {
    ...
    try {
        ...
        if (response.success && response.data) {
            ...
            toast.add({ color: 'error', title: 'Erro ao carregar os detalhes do pedido' });
...
onMounted(async () => {
    try {
        const profileResponse = await $fetch<{ success: boolean; data: any }>(`/api/bikers/me`);
