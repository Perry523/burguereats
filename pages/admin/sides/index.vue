      ...
      <template #filter>
        <div class="w-full px-5 pb-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <NuxtLink to="/admin/sides/create" rel="noopener noreferrer">
            <button class="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary shadow-sm whitespace-nowrap">
              Adicionar Categoria
            </button>
          </NuxtLink>
        </div>
      ...
      <template #required="{ row }">
        <span
          :class="[
            ...
            row.isRequired ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700'
          ]"
        >
...
const totalPages = computed(() => Math.max(1, Math.ceil(filteredSideCategories.value.length / itemsPerPage.value)))
