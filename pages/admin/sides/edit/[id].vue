<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold text-gray-800">Editar Categoria de Acompanhamento</h1>
        <p class="text-sm text-gray-500">Atualize as informações da categoria</p>
      </div>
    ...
    <div v-if="isLoading" class="space-y-4">
      <div class="h-8 w-1/3 animate-pulse rounded-lg bg-gray-200"></div>
      <div class="h-32 w-full animate-pulse rounded-lg bg-gray-100"></div>
      <div class="h-8 w-1/2 animate-pulse rounded-lg bg-gray-200"></div>
    </div>

    <div v-else-if="category" class="max-w-4xl rounded-lg border border-gray-200 bg-white shadow-sm">
      <form @submit.prevent="saveCategory" class="space-y-6 p-6">
        <div class="grid gap-6 md:grid-cols-2">
          <div>
            ...
            <input
              ...
              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20"
            />
          </div>
          <div>
            ...
            >
              <div class="mb-4 flex items-start justify-between">
                <h4 class="font-medium text-gray-900">
                  {{ side.id ? `Item: ${side.name || 'Sem nome'}` : `Novo Item ${index + 1}` }}
                </h4>
                <button
                  type="button"
                ...
                <div class="md:w-48 md:flex-shrink-0">
                  ...
                  <div v-if="side.imagePreview" class="space-y-3">
                    <div class="relative aspect-square overflow-hidden rounded-lg bg-gray-100">
                      <img :src="side.imagePreview" alt="Pré-visualização do item" class="h-full w-full object-cover" />
                      <button
                        type="button"
                ...
                <div class="flex-1 space-y-4">
                  ...
                  <label class="inline-flex items-center gap-3 text-sm font-medium text-gray-700">
                    ...
                    <span
                      ...
                      :class="side.isAvailable ? 'bg-orange-500' : 'bg-gray-300'"
                    >
                      <span
        ...
        <div class="flex justify-end gap-4 border-t border-gray-200 pt-6">
          ...
          <button
            type="submit"
            class="rounded-lg bg-orange-500 px-4 py-2 text-sm font-medium text-white hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-60"
            :disabled="isSaving"
          >
            {{ isSaving ? 'Atualizando...' : 'Atualizar Categoria' }}
          </button>
        </div>
      </form>
    </div>
    ...
    <div v-else class="py-12 text-center">
      ...
      <NuxtLink to="/admin/sides">
        <button
          type="button"
          class="mt-4 rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100"
        >
          Voltar para Lista
        </button>
      </NuxtLink>
...
const createSideItem = (initial?: Partial<SideItemForm>): SideItemForm => ({
  id: initial?.id,
  name: initial?.name ?? '',
  description: initial?.description ?? '',
  priceIncrement:
    typeof initial?.priceIncrement === 'number' && Number.isFinite(initial.priceIncrement)
      ? initial.priceIncrement
      : 0,
  image: initial?.image ?? '',
  isAvailable: initial?.isAvailable ?? true,
  imageFile: null,
  imagePreview: initial?.image?.trim() || null,
  imageError: null,
  objectUrl: null,
  isDragging: false,
})
...
const category = ref<SideCategoryRecord | null>(null)
...
const applyFetchedSides = (items: SideItemRecord[]) => {
  form.sides.forEach(cleanupSideImage)
  const mapped = items.map(side =>
    createSideItem({
      id: side.id,
      name: side.name,
      description: side.description ?? '',
      priceIncrement: Number(side.priceIncrement),
      image: side.image ?? '',
      isAvailable: side.isAvailable,
    })
  )
  form.sides.splice(0, form.sides.length, ...mapped)
}
...
const fetchCategory = async () => {
  try {
    const response = await $fetch<{ success: boolean; data?: SideCategoryRecord }>(`/api/side-categories/${categoryId}`)
    if (response.success && response.data) {
      category.value = response.data
      form.name = response.data.name
      form.description = response.data.description ?? ''
      form.isRequired = response.data.isRequired
      form.maxSelections = response.data.maxSelections
      form.order = response.data.order
      applyFetchedSides(response.data.sides)
    }
  } catch (error) {
...
const resolveCompanyId = () => category.value?.companyId || authStore.user?.company?.id || ''
...
const saveCategory = async () => {
  ...
  try {
    ...
    await $fetch(`/api/side-categories/${categoryId}`, {
      method: 'PUT',
      body: payload,
    })
