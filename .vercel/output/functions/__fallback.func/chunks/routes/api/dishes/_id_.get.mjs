...
const _id__get = defineEventHandler(async (event) => {
  try {
    ...
    const dishCategories = await db.db("DishCategory").join("Category", "DishCategory.categoryId", "Category.id").where("DishCategory.dishId", id).select("Category.*");
    const dishSideCategories = await db.db("DishSideCategory").join("SideCategory", "DishSideCategory.sideCategoryId", "SideCategory.id").where("DishSideCategory.dishId", id).select("SideCategory.*", "DishSideCategory.order as dishOrder");
    return {
      success: true,
      data: {
        ...dish,
